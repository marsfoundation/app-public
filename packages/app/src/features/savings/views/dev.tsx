...
export function Dev() {
  ...
  return (
    <div>
      <p className="m-2 text-md">Block timestamp: {blockTimestamp?.toISOString() ?? '-'}</p>
      ...
      <button onClick={() => rho.refetch()}>REFETCH</button>
    </div>
  )
}

export function useMulticallBlockTimestamp(): Date | undefined {
  ...
  return new Date(Number(response.data) * 1000)
}
